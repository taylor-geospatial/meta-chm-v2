...
import * as CHMAnalytics from "./analytics.js";

// maplibregl + GeoTIFF are UMD globals (loaded via <script> in index.html — keeping them
// out of the bundle dodges esm.sh worker-bundling failures).
const { maplibregl, GeoTIFF } = window;
...
const aoi = { drawing: false, verts: [], ring: null };
...
async function cogDecodeR(z, x, y) {
  ...
  const side = overview ? L.width / 2 ** z : L.width / s10;
...
async function computeMetrics() {
  ...
  for (let i = 0; i < tiles.length; i++) {
    ...
    for (let ty = 0; ty < 256; ty++) {
      let dst = (oy + ty) * W + ox;
      let src = ty * 256;
      for (let tx = 0; tx < 256; tx++, dst++, src++) mosaic[dst] = px[src];
    }
  }
  if (!any) {
    body.className = "muted";
...
function drawHist(counts, lo, hi) {
  ...
  const top = Math.min(50, counts.length);
  let peak = 1;
...
function onAOIDblClick(e) {
  if (!aoi.drawing) return;
  e.preventDefault();
  if (aoi.verts.length >= 4) aoi.verts.pop();
  finishDraw();
}

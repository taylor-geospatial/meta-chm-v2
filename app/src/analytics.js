...
// Pure math, no DOM.
//
// Pixel semantics (mirrors the colorization LUT in main.js):
...
export function summarize(heights, forestThr) {
...
export function rumple(heights, w, h, mpp) {
  let sum = 0;
  let n = 0;
  for (let y = 0; y < h; y++) {
    const row = y * w;
...
export function gaps(heights, w, h, forestThr, mpp, minGapPx) {
  const n = w * h;
  const open = new Uint8Array(n);
  for (let i = 0; i < n; i++) open[i] = heights[i] < forestThr ? 1 : 0;
...
export function edges(heights, w, h, forestThr, mpp) {
  const n = w * h;
  let forest = 0;
  let edgePx = 0;
  let segments = 0;
  ...
  for (let y = 0; y < h; y++) {
    ...
    for (let x = 0; x < w; x++) {
      const cur = heights[row + x];
      if (cur <= 254) observed++;
      if (cur < forestThr || cur > 254) continue;
      forest++;
      let seg = 0;
      let v;
...
export function clipToPolygon(heights, w, h, ringPx) {
  ...
  for (let y = 0; y < h; y++) {
    ...
    for (let k = 0; k + 1 < xs.length; k += 2) {
      ...
      for (; x < xa; x++) heights[row + x] = NODATA;
      if (xb >= x) x = xb + 1;
    }
    for (; x < w; x++) heights[row + x] = NODATA;
  }
}

// Run the full suite over a mosaic; returns metrics + a per-stage timing breakdown (ms).
export function analyzeMosaic(heights, w, h, opts) {

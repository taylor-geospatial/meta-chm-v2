...
# Toggle the WorldCover "Forest only" mask and confirm metrics recompute against tree pixels.
DRIVE_MASK_JS = """
async () => {
  const sleep = (ms) => new Promise((r) => setTimeout(r, ms));
  for (let i = 0; i < 80 && !window.__map; i++) await sleep(150);
  if (typeof window.__clearAOI === "function") window.__clearAOI();
  const map = window.__map;
  await new Promise((res) => {
    map.once("idle", res);
    map.jumpTo({ center: [8.21, 48.27], zoom: 13 });
    setTimeout(res, 8000);
  });
  const cb = document.getElementById("wc-mask");
  cb.checked = true;
...
def test_worldcover_forest_mask(page):
    ...
    m = re.search(r"Tree cover \(WorldCover\)\s*\n?\s*([\d.]+)%", text)
    assert m, f"no WorldCover tree-cover stat in:\n{text}"
    pct = float(m.group(1))
    assert 0 <= pct <= 100, f"tree cover {pct}% out of range"
    # Black Forest is densely treed — sanity-check the alignment is right, not inverted
    assert pct > 40, f"expected high tree cover in Black Forest, got {pct}%"
